...
<AccordionGroup>
  <Accordion title="POST /hooks/wake">
    ...
    HTTP `200` includes `eventOutcome: "queued"` when a new event was enqueued or `eventOutcome: "coalesced"` when an equivalent event was already pending. With `mode: "now"`, a wake is requested in either case; the response does not mean a heartbeat completed. Use `mode: "next-heartbeat"` to avoid requesting an immediate wake.
  ...
  <Accordion title="Mapped hooks (POST /hooks/<name>)">
    ...
    `forEach: "<key>"` fans out over a top-level payload array. Each item sees a one-element array, so the Gmail preset's `messages[0]` means the current email. Agent fan-out admission answers after at most about 8 seconds of dispatch waiting; pending items continue in the background and a partial batch returns non-2xx. Retrying the same batch reuses pending or admitted agent items while the bounded in-memory replay cache retains them. It is not durable exactly-once delivery; mapped wake actions have no replay identity, and equivalent pending system events may be coalesced. The reference covers batch caps and response shapes.
