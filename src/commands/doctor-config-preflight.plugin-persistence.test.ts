...
describe("Doctor plugin persistence", () => {
    ...
    async ({ scope, replaceBeforeLease }) => {
      await withPreflightPluginFixture(async (writeVersion) => {
        const run = async () => {
          ...
          const siblingLease = replaceBeforeLease ? acquireStartupMigrationLease() : undefined;
          let replaced = false;
          try {
            ...
            expect(hasActiveStartupMigrationLease({ env: process.env })).toBe(false);
  ...
  it.each([false, true])(
    "records the state checkpoint only after durable verification (interrupted=%s)",
    async (interrupted) => {
      await withPreflightPluginFixture(async () => {
        ...
        const operation = runDoctorConfigPreflight({
          ...
          measure: async (name, run) => {
            ...
            if (name === "doctor.config-preflight.config-snapshot" && persisted) {
              ...
              expect(needsStateMigrationCheckpoint({ identity })).toBe(true);
              verified = true;
              if (interrupted) {
        ...
        if (interrupted) {
          await expect(operation).rejects.toThrow("verification interrupted");
        } else {
        ...
        expect(needsStateMigrationCheckpoint({ identity })).toBe(interrupted);
        expect(hasActiveStartupMigrationLease({ env: process.env })).toBe(false);
      });
    },
  );
